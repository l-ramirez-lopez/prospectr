...
<<binning,fig.cap="Average in bins",fig.height=6,fig.width=10, tidy=TRUE, dpi=300, fig.pos='t'>>=
...
matplot(as.numeric(colnames(NIRsoil$spc)),t(NIRsoil$spc[1:10,]),type="l",ylim=c(0,.6),xlab="Wavelength /nm",ylab="Absorbance")
...
matplot(as.numeric(colnames(NIRsoil$spc)),t(NIRsoil$spc[ho$model,]),type="l",xlab="Wavelength",ylab="Absorbance")
# add bands used during the selection process
abline(v=as.numeric(colnames(NIRsoil$spc))[ho$bands],lty=2)
@
  
\bibliographystyle{plain}
\bibliography{prospectr-intro}
