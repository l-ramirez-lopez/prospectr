...
We show below how they can be used, using the Near-Infrared (NIR) dataset (NIRsoil) included in the package [@fernandez2008]. Observations should be arranged row-wise.
...
```{r msc, fig.cap="Effect of MSC on raw spectra",fig.height=6,fig.width=10, tidy=TRUE, dpi = 50,out.height="60%",out.width="100%"}
...
axis(4, col = "red")
grid()
legend("topleft", legend = c("raw", "MSC signal"), lty = c(1, 1),col = c("black", "red"))
par(new = FALSE)
```
...
plot(as.numeric(colnames(NIRsoil$spc)), 
     NIRsoil$spc[1,], type = "l", xlab = "Wavelength", ylab = "Absorbance", lwd = 1.5)
par(new = TRUE)
