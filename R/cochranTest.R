...
#' \itemize{
#'  \item{'`X`'}{ input `matrix` from which outlying observations (rows) have been removed}
#'  \item{'`outliers`'}{ numeric `vector` giving the row indices of the input data that have been flagged as outliers}
#' }
#' @details
#' The Cochran *C* test is test whether a single estimate of variance is significantly larger than a a group of variances.
...
#'
#' For multivariate data, the variance \eqn{S_i^2} can be computed on aggregated data, using a summary function (`fun` argument)
#' such as `sum`, `mean`, or first principal components ('PC1' and 'PC2').
#'
#' An observation is considered to have an outlying variance if the Cochran *C* statistic is higher than an upper limit critical value \eqn{C_{UL}}
#' which can be evaluated with ('t Lam, 2010):
#'
#' \deqn{ C_{UL}(\alpha,n,N) = \left [1+\frac{N-1}{F_{c}(\alpha/N,(n-1),(N-1)(n-1))} \right ]^{-1}}
#'
#' where \eqn{\alpha} is the *p*-value of the test, \eqn{n} is the (average) number of replicates and \eqn{F_c} is the critical value of the Fisher's \eqn{F} ratio.
#'
#' The replicates with outlying variance are removed and the test can be applied iteratively until no outlying variance is detected under the given *p*-value.
#' Such iterative procedure is implemented in `cochranTest`, allowing the user to specify whether a set of replicates
