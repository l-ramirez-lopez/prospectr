...
#' @param response a logical indicating if the data of the response variables must be returned (default is `TRUE`).
...
#' @return a `data.frame` containing the metadata, response variables (if `response = TRUE`) and spectra (if `spectra = TRUE`, embedded in the `data.frame` as a `matrix` named `...$spc`).
#' @details
