...
#' @param responsevar a logical indicating if the data of the response variables must be returned (default is `TRUE`).
#' @param spectra a logical indicating if the spectral data must be returned  (default is `TRUE`).
#' @param metadata a logical indicating if the metadada must be returned  (default is `TRUE`).
#' @param progress a logical indicating if a progress bar must be printed (default is `TRUE`).
#' @param verbose a logical indicating if the number of spectra and response variables (an also the ID's of the spectra without gain and/or temperature information) must be printed (default is `TRUE`).
#' @return a `data.frame` containing the metadata, response variables (if `responsevar = TRUE`) and spectra (if `spectra = TRUE`, embedded in the `data.frame` as a `matrix` named `...$spc`).
#' @details
