...
#' the distance in the standardized Principal Component space (Mahalanobis distance).
#' If `pc < 1`, the number of principal components kept corresponds to the
#' number of components
#' explaining at least (`pc * 100`) percent of the total variance (default = 0.95
#' as in the Puchwein paper).
...
#' \itemize{
#'  \item{'`model`'}{ indices of the observations (row indices of the input data)
...
#' matrix is then normalized to unit variance and the Euclidean distance of each sample
#' to the centre of the data is computed, which is identical to the Mahalanobis
#' distance \eqn{H}. Additionally, the Mahalanobis distances between samples are
#' comptuted. The algorithm then proceeds as follows:
#'
#' \enumerate{
#'    \item Choose a initial limiting distance \eqn{d_{ini}}
#'    \item Select the sample with the highest \eqn{H} distance to the centre
#'    \item Remove all samples within the minimum distance \eqn{d_{ini}} from the
...
#' is considered as producing the optimal selection of points (the subset that
#' best reproduces the variability of the predictor space).
#' @note The Puchwein algorithm is an iterative method and can be slow for large
#' data matrices.
#' @seealso \code{\link{kenStone}}, \code{\link{duplex}}, \code{\link{shenkWest}},
#' \code{\link{honigs}}, \code{\link{naes}}
#' @export
